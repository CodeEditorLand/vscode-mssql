...
gulp.task("ext:generate-runtime-localization-files", async function () {
	...
	for (const xliffFile of xliffFiles) {
		...
		const xliffFileContents = await fs.readFile(
			path.resolve("./localization/xliff", xliffFile),
			"utf8",
		);
		...
		for (fileContent of l10nDetailsArrayFromXlf) {
			if (fileContent.name === "bundle") {
				...
				await fs.writeFile(
					filePath,
					JSON.stringify(fileContent.messages, null, 2),
				);
			} else if (fileContent.name === "package") {
				if (fileContent.language === "enu") {
					// We don't need the enu nls file as it is edited manually by us.
					continue;
				}
...
gulp.task("ext:generate-xliff-files", async function () {
	const enuXlifFile = await fs.readFile(
		"./localization/xliff/enu.xlf",
		"utf8",
	);
	...
	for (const lang of supportedLocLangs) {
		...
		const existingXlifFile = await fs.readFile(
			`./localization/xliff/${lang}.xlf`,
			"utf8",
		);
		...
		for (const resourceType of resourceTypes) {
			for (const key of Object.keys(
				copyOfEnuXlif.resources[resourceType],
			)) {
...
gulp.task("ext:extract-localization-strings", async function () {
	...
	map.set(
		"package",
		JSON.parse(await fs.readFile(path.resolve("package.nls.json"), "utf8")),
	);
 ...
 */
gulp.task(
	"ext:generate-pseudo-loc",
	gulp.series("ext:extract-localization-strings", async function () {
		const bundle = await getL10nJson();
		const packageJson = await fs.readFile(
			path.resolve("package.nls.json"),
			"utf8",
		);
		const pseudoLocBundle = await vscodel10n.getL10nPseudoLocalized(bundle);
		const pseudoLocPackage = await vscodel10n.getL10nPseudoLocalized(
